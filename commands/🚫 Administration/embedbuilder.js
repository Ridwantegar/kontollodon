...
let buttonEvent = async (interaction) => {
  ...
  if (interaction?.message.id === msg.id) {
    if (interaction?.user.id === message.member.id) {
      ...
      await interaction?.reply({content : `<:945226710394232852:1167791114883956746>  You are not allowed to do that! Only: <@${message.author.id}>`, ephemeral : true}); //ephemeral message
    }
  }
}
