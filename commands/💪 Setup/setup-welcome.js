      ...
      async function handle_the_picks(optionhandletype, SetupNumber, menuoptiondata) {
        switch (optionhandletype) {
                  ...
                  async function handle_the_picks_3(optionhandletype, SetupNumber, menuoptiondata) {
                    switch (optionhandletype) {
                      ...
                      case `Frame Color`: {
                        ...
                        //Once the Collections ended edit the menu message
                        collector.on('end', collected => {
                          tempmsg.edit({ embeds: [tempmsg.embeds[0].setDescription(`~~${tempmsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().customId ? `<:945226678362316841:1167791111184592988>  **Selected the \`${collected.first().customId}\` Color**` : "❌ **NOTHING SELECTED - CANCELLED**"}` })
                        });
                        //Menu Collections
          ...
          case `Test Welcome`: {
            ...
            let invitemessage = `Invited by ${member.user.tag ? `**${member.user.tag}**` : `<@${member.user.id}>`}\n<:Like:857334024087011378> **X Invites**\n[<:joines:866356465299488809> X Joins | <:leaves:866356598356049930> X Leaves | <:945226710394232852:1167791114883956746>  X Fakes]`
            if (welcome) {
