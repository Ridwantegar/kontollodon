        ...
        switch (optionhandletype) {
            ...
            async function third_layer(SetupNumber) {
              ...
              collector.on('end', collected => {
                menumsg.edit({
                  embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                  components: [],
                  content: `${collected && collected.first() && collected.first().values ? `<:945226678362316841:1167791111184592988>  **Selected: \`${collected ? collected.first().values[0] : "Nothing"}\`**` : "❌ **NOTHING SELECTED - CANCELLED**"}`
                })
              });
            }
          ...
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<:945226710394232852:1167791114883956746>  **You need to add at least 1 Open-Ticket-Option**")
            }
          ...
          case "Edit Ticket Option": {
            ...
            let data = theDB.get(message.guild.id, pre + ".data");
            if (!data || data.length < 1) {
              return message.reply("<:945226710394232852:1167791114883956746>  **There are no Open-Ticket-Options to remove**")
            }
            let embed = new MessageEmbed()
              .setColor(es.color)
              .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                ...
                async function handle_the_picks3(optionhandletype) {
                  ...
                  switch (optionhandletype) {
                    ...
                    case `Change Open Category`: {
                      ...
                      return message.reply("<:945226710394232852:1167791114883956746>  **Invalid Category-ID added**")
                    } break;
            ...
            collector.on('end', collected => {
              menumsg.edit({
                embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                components: [],
                content: `<:945226678362316841:1167791111184592988>  **Selected: \`${collected.size > 0 ? collected.first().values[0] : "NOTHING"}\`**`
              })
            });
          }
            break;
