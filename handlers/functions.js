...
function GetUser(message, arg){
  var errormessage = "<:icon_disabled:945226710394232852>  I failed finding that User...";
...
function GetRole(message, arg){
  var errormessage = "<:icon_disabled:945226710394232852>  I failed finding that Role...";
  return new Promise(async (resolve, reject) => {
    var args = arg, client = message.client;
    if(!client || !message) return reject("CLIENT IS NOT DEFINED")
    if(!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
 ...
 */
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== message.author.id)
        return b?.reply({content: `<:icon_disabled:945226710394232852>  **Only the one who typed ${prefix}help is allowed to react!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {
